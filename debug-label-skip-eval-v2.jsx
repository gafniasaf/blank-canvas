...
function isLikelyLabelParagraph(para) {
  try {
    ...
    if (w(b) <= MAX_LABEL_FRAME_W && h(b) <= MAX_LABEL_FRAME_H) {
      ...
      var isBodyish = (sLow.indexOf("basis") !== -1) || (sLow.indexOf("_bullets") !== -1) || (sLow.indexOf("bullet") !== -1);
      if (isBodyish) {
        if (t.length <= 40) return true;
        return false;
      }
      if (t.length <= MAX_LABEL_PARA_CHARS) return true;
    }
  } catch (e1) {}
  return false;
}
...
var targets = [210, 211, 212, 261, 263];
var outPath = Folder.desktop.fsName + "/debug_label_skip_eval_v2_" + isoStamp() + ".txt";
