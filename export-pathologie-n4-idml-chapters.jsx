// ============================================================
// EXPORT: Pathologie N4 chapter INDDs (Downloads) → IDMLs (designs-relinked/)
// ============================================================
// Purpose:
// - Pathologie N4 is delivered as an .indb + per-chapter .indd files.
// - Our DB ingest pipeline works from IDML, so we export each chapter document to IDML.
//
// Output:
// - Writes IDMLs to:
//   <repo>/designs-relinked/MBO Pathologie nivo 4_9789083412016_03/<same>.idml
// - Writes a Desktop report: export_pathologie_n4_idml_chapters__<timestamp>.txt
//
// SAFE:
...
// Run:
// - From AppleScript:
//   tell application "Adobe InDesign 2026" to do script (POSIX file "<repo>/export-pathologie-n4-idml-chapters.jsx") language javascript
// ============================================================
...
(function () {
  ...
  var out = [];
  out.push("=== EXPORT PATHOLOGIE N4 CHAPTER IDMLs ===");
  ...
  if (!repoRoot || !repoRoot.exists) {
    ...
    writeTextToDesktop("export_pathologie_n4_idml_chapters__" + isoStamp() + ".txt", out.join("\n"));
    alert("Could not resolve repo root.");
    return;
  }
  out.push("Repo: " + repoRoot.fsName);

  var inddDir = "/Users/asafgafni/Downloads/MBO 2024/Binnenwerk/MBO Pathologie nivo 4_9789083412016_03";
  var outDir = repoRoot.fsName + "/designs-relinked/MBO Pathologie nivo 4_9789083412016_03";
  var copyDir = repoRoot.fsName + "/designs-relinked/_tmp_copies/MBO Pathologie nivo 4_9789083412016_03";
  ...
  for (var ch = 1; ch <= 12; ch++) {
    var chStr = pad2(ch);
    var inddPath = inddDir + "/Pathologie_mbo_CH" + chStr + "_03.2024.indd";
    var outIdmlPath = outDir + "/Pathologie_mbo_CH" + chStr + "_03.2024.idml";
    var copyInddPath = copyDir + "/Pathologie_mbo_CH" + chStr + "_03.2024__COPY.indd";
  ...
  try {
    if (failed === 0) alert("✅ Exported Pathologie N4 chapter IDMLs. See Desktop report for details.");
