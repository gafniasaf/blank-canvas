// inspect-ch1-parastyles.jsx
// Read-only: Report paragraph style distribution in CH1 body story range.
...
(function () {
  ...
  if (app.documents.length === 0) {
    ...
    if (body.index >= 0) {
      ...
      var entries = [];
      for (var k in counts) if (counts.hasOwnProperty(k)) entries.push({ style: k, count: counts[k] });
      entries.sort(function (a, b) { return b.count - a.count; });

      out.push("Total paragraphs in CH1 body story range: " + total);
      out.push("Unique paragraph styles: " + entries.length);
      out.push("");
      out.push("Top styles:");
      for (var i = 0; i < Math.min(40, entries.length); i++) {
        out.push(" - " + entries[i].style + ": " + entries[i].count);
      }
    }
  }
  ...
  writeTextToDesktop("inspect_ch1_parastyles__" + safeFileName((app.documents.length ? app.activeDocument.name : "no_doc")) + "__" + isoStamp() + ".txt", report);
  report;
})();
