// ============================================================
// FIX: trim trailing empty pages (generic, chapter output)
// ============================================================
// Removes empty pages at the END of the document that are likely artifacts of
...
(function () {
  ...
  if (app.documents.length === 0) {
    ...
    writeTextToDesktop("trim_trailing_empty_pages__no_doc__" + isoStamp() + ".txt", out.join("\n"));
    return out.join("\n");
  }
  ...
  out.push("Pages before: " + doc.pages.length);
  out.push("");

  if (body.index < 0) {
    out.push("ERROR: could not detect body story");
    writeTextToDesktop("trim_trailing_empty_pages__" + safeFileName(doc.name) + "__" + isoStamp() + ".txt", out.join("\n"));
    return out.join("\n");
  }
  ...
  out.push("Pages removed: " + removed);
  out.push("Pages after:  " + doc.pages.length);
  if (samples.length) out.push("Samples: " + samples.join(" | "));

  var report = out.join("\n");
  writeTextToDesktop("trim_trailing_empty_pages__" + safeFileName(doc.name) + "__" + isoStamp() + ".txt", report);
  report;
})();
